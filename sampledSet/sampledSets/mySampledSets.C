...
namespace Foam
{
    defineTypeNameAndDebug(mySampledSets, 0);
...
bool Foam::mySampledSets::verbose_ = false;


// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::mySampledSets::combineSampledSets
(
...
)
{
    ...
    const PtrList<mySampledSet>& mySampledSets = *this;

    forAll(mySampledSets, setI)
    {
        labelList segments;
        masterSampledSets.set
        (
            setI,
            mySampledSets[setI].gather(indexSets[setI], segments)
        );
    }
}
...
Foam::mySampledSets::mySampledSets
(
    const word& name,
    const Time& runTime,
...
void Foam::mySampledSets::verbose(const bool verbosity)
{
    verbose_ = verbosity;
}


bool Foam::mySampledSets::execute()
{
    return true;
}


bool Foam::mySampledSets::write()
{
    if (size())
    {
...
bool Foam::mySampledSets::read(const dictionary& dict)
{
    dict_ = dict;
    ...
    if (dict_.found("sets"))
    {
        ...
        PtrList<mySampledSet> newList
        (
            dict_.lookup("sets"),
            mySampledSet::iNew(mesh_, searchEngine_)
        );
        transfer(newList);
...
void Foam::mySampledSets::correct()
{
    if (dict_.found("sets"))
    {
...
void Foam::mySampledSets::updateMesh(const mapPolyMesh& mpm)
{
    if (&mpm.mesh() == &mesh_)
    {
        correct();
...
void Foam::mySampledSets::movePoints(const polyMesh& mesh)
{
    if (&mesh == &mesh_)
    {
        correct();
...
void Foam::mySampledSets::readUpdate(const polyMesh::readUpdateState state)
{
