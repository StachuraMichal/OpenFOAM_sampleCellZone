...
namespace Foam
{
    defineTypeNameAndDebug(mySampledSet, 0);
    defineRunTimeSelectionTable(mySampledSet, word);
}


// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

void Foam::mySampledSet::checkDimensions() const
{
    if
    (
        (cells_.size() != size())
...
Foam::label Foam::mySampledSet::getBoundaryCell(const label facei) const
{
    return mesh().faceOwner()[facei];
}


Foam::label Foam::mySampledSet::getNeighbourCell(const label facei) const
{
...
Foam::label Foam::mySampledSet::pointInCell
(
    const point& p,
    const label samplei
) const
...
Foam::scalar Foam::mySampledSet::calcSign
(
    const label facei,
    const point& sample
) const
...
Foam::label Foam::mySampledSet::findNearFace
(
    const label celli,
    const point& sample,
...
Foam::point Foam::mySampledSet::pushIn
(
    const point& facePt,
    const label facei
) const
...
bool Foam::mySampledSet::getTrackingPoint
(
...
void Foam::mySampledSet::setSamples
(
...
Foam::autoPtr<Foam::coordSet> Foam::mySampledSet::gather
(
    labelList& indexSet,
...
Foam::mySampledSet::mySampledSet
(
    const word& name,
    const polyMesh& mesh,
...
Foam::autoPtr<Foam::mySampledSet> Foam::mySampledSet::New
(
    const word& name,
    const polyMesh& mesh,
    const meshSearch& searchEngine,
...
Foam::Ostream& Foam::mySampledSet::write(Ostream& os) const
{
    coordSet::write(os);
